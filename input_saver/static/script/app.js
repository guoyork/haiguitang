        ...
        2. 提示应该简短明了` },
            ...
            async function loadPuzzle(fileName) {
                try {
                    ...
                    const response = await fetch(`/puzzles/?name=${fileName}`);
        ...
        4. 如果问题与谜题无关回答"{没有关系}"` },
                    ...
                    fetch('/save/', {
                        ...
                        body: JSON.stringify({'name': puzzleFile, 'question': question, 'answer': answer})
                    });
        ...
        document.querySelectorAll('.stars label').forEach(star => {
            ...
            star.addEventListener('click', function() {
                ...
                document.querySelectorAll('.stars label').forEach(s => {
                    if (s === clickedStar) found = true;
                    s.style.color = found ? 'gold' : '#ccc';
                });
            });
            ...
            star.addEventListener('mouseout', function() {
                ...
                document.querySelectorAll('.stars label').forEach(s => {
                    s.style.color = (checked && s.htmlFor <= checked.id) ? 'gold' : '#ccc';
                });
            });
        });
